...
const HowItWorks: React.FC<HowItWorksProps> = ({ data }) => {
  ...
  return (
    <section className="py-20 px-4 sm:px-6 lg:px-8" style={{ backgroundColor: data.color_theme?.background_color || '#F9FAFB' }}>
      <div className="max-w-7xl mx-auto relative">
        {hasMultipleSteps && (
          <button
            onClick={() => setCurrentStep((prev) => Math.max(0, prev - 1))}
            disabled={!canGoPrev}
            className="absolute -left-16 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white shadow-lg flex items-center justify-center z-10"
            style={{
        ...
        {hasMultipleSteps && (
          <button
            ...
            disabled={!canGoNext}
            className="absolute -right-16 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white shadow-lg flex items-center justify-center z-10"
            style={{
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          ...
          <div className="relative">
            ...
            <div
              className="absolute top-6 right-6 text-white px-6 py-4 rounded-xl shadow-lg"
                      ...
                      {step.content?.map((item, idx) => (
                        <div key={idx} className="flex gap-4 pl-4">
                          ...
                          <div className="flex-1">
                            <h4 className="text-xl font-bold mb-2" style={{ color: data.color_theme?.text_color || '#111827' }}>
                              {item.title}
                            </h4>
                            <p className="leading-relaxed" style={{ color: data.color_theme?.neutral_color || '#6B7280' }}>
                              {item.description}
                            </p>
                          </div>
                        </div>
